var hyperHTML = (function (document) {
  ...
  var indexOf$1 = function indexOf(
    moreNodes,
    moreStart,
    moreEnd,
    lessNodes,
  ...
  var HS = function HS(
    ...
    var minLen =
      futureChanges < currentChanges ? futureChanges : currentChanges;
  ...
  var applyDiff = function applyDiff(
    diff,
    get,
    parentNode,
    futureNodes,
  ...
  var smartDiff = function smartDiff(
    get,
    parentNode,
    futureNodes,
    futureStart,
  ...
  function disconnected(poly) {
    ...
    function startObserving(document) {
      ...
      function dispatchAll(nodes, type, wsin, wsout) {
        for (
          var node, event = new Event(type), length = nodes.length, i = 0;
          i < length;
  ...
  try {
    if (
      !(function (template, content, tabindex) {
        return (
          content in template &&
    ...
    while (i < length) {
      ...
      if (direct || 1 < (sparse = attribute.value.split(UIDC)).length) {
        ...
        if (cache.indexOf(name) < 0) {
          ...
          var value =
            attributes[realName] || // the following ignore is covered by browsers
  ...
  function createDetails(options, template) {
    var info =
      parsed.get(template) ||
      parsed.set(template, createInfo(options, template));
  ...
  var _templateLiteral = function templateLiteral(tl) {
    ...
    var FTS =
      !(RAW in tl) || tl.propertyIsEnumerable(RAW) || !Object.isFrozen(tl[RAW]);
    ...
    if (broken || FTS) {
      ...
      if (FTS) _templateLiteral = foreverCache;
