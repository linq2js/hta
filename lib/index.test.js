...
test("parse no substitution HTML", () => {
  const result = parseTemplate(tag`<a></a>`);
  expect(result.html).toBe("<a></a>");
});
...
test("parse HTML contains 1 directive", () => {
  const result = parseTemplate(tag`<a ${true}></a>`);
  expect(result.html).toBe(`<a  hta-0="1" ></a>`);
});
...
test("parse HTML contains multiple directives", () => {
  ...
  expect(result.html).toBe(
    `<a  hta-0="1" ><span  hta-1="1"   hta-2="1" ></span></a>`
  );
});
...
test("parse HTML contains 1 placeholder", () => {
  const result = parseTemplate(tag`<a>${true}</a>`);
  expect(result.html).toBe(
    `<a><template hta-slot="1" hta-0="1"></template></a>`
  );
});
...
test("$.state", async () => {
  ...
  const Counter = () => {
    const [count1, setCount1] = $.state(1);
    const [count2, setCount2] = $.state(2);
    rerender();
...
test("counter", () => {
  const CountValue = () => $`<h1>${$.store((state) => state.count)}</h1>`;
  const CountAction = () =>
    $`<button ${{
...
test("suspense", async () => {
  const AsyncValue = () =>
    $`<h1>${$.store((state, valueOf) => valueOf(state.asyncValue))}</h1>`;
...
test("consume custom context", () => {
  ...
  render(
    () => $`
    ${$.provide("c1", "b1", Counter1)}
    ${$.provide("c1", "b2", Counter2)}
    ${$.provide("c2", "b3", Counter1)}
    ${$.provide("c2", "b4", Counter2)}`
  );

  expect(query("#b1")).not.toBeNull();
  expect(query("#b2")).toBeNull();
  expect(query("#b3")).toBeNull();
  expect(query("#b4")).not.toBeNull();
});
