...
function getStringArrayCacheKey(strings) {
  let i = stringArrayCache.length;
  while (i--) if (arrayEqual(stringArrayCache[i], strings)) return i;
  stringArrayCache[stringArrayCache.length] = strings;
  return stringArrayCache.length - 1;
}

function createTemplate(key, strings, values) {
  let parsedHtml;
  let cacheKey = getStringArrayCacheKey(strings);
  return {
    type: templateType,
    key,
    strings,
    ...
    render(marker, newTemplate, renderResult) {
      ...
      if (!parsedHtml) {
        parsedHtml = htmlCache[cacheKey];
        if (!parsedHtml)
          htmlCache[cacheKey] = parsedHtml = parseTemplate(strings);
      }
...
function renderTemplate(context, shared, marker, template) {
  tryUnmount(
    marker,
    (c) => c.type !== templateType || c.template.cacheKey !== template.cacheKey
  );
  let isNew = false;
