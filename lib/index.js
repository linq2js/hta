...
const htmlType = () => {};
const loadableType = () => {};
const loadablePromiseType = () => {};
const rawType = () => {};
const templateCache = {};
const bindingPrefix = "hta-";
const defaultItemKey = (value, index) => index;
...
const memoHookType = 1;
const callbackHookType = 2;
const effectHookType = 3;
const enqueue =
...
function createTemplate(strings) {
  ...
  for (let i = 0; i < slotCount; i++) {
    ...
    const attr = ` ${bindingPrefix}${i}="1" `;
    if (subs && subs.type === rawType) {
      htmlArray.push(`<template ${attr}></template>`);
    } else {
      htmlArray.push(attr);
    }
  ...
  const instance = {
    ...
    substitutions,
    isEqual(template) {
      return arrayEqual(template.strings, strings);
    },
    render(container) {
...
function bind(context, container, data, binding) {
  ...
  if (binding.type === rawType) {
    return bindRaw(context, container, data, binding);
  }

  return applyBinding(context, container, data, binding);
}

function bindRaw(context, container, data, binding) {
  const value = "" + binding.value;
  if (container.$$value === value) return;
  container.$$value = value;
  if (container.$$children) {
    if (container.parentNode) {
      for (let i = 0; i < container.$$children.length; i++) {
        container.parentNode.removeChild(container.$$children[i]);
      }
    }
  }
  container.innerHTML = value;
  container.$$children = [...container.content.childNodes];
  if (container.parentNode) {
    for (let i = 0; i < container.$$children.length; i++) {
...
function bindList(context, container, list, itemOptions) {
  ...
  if (!itemOptions || typeof itemOptions === "function") {
    ...
    data.item = {
      ...
      key: itemOptions.key || defaultItemKey,
  ...
  if (data.children.length && !list.length) {
    ...
    if (newCount === list.length) {
      ...
      if (bindings.length) {
        if (updateAsync) {
          function update() {
            if (updateToken !== data.updateToken) return;
            bindMultiple(bindings.splice(0, 500));
            if (bindings.length) enqueue(update);
          }
          bindings.length <= 500 ? update() : enqueue(update);
        } else {
...
function mount(context, container, data, component, props = {}) {
  initComponent(context, container, data, component);
  data.update(props);
}

function initComponent(context, container, data, component) {
  ...
  data.mounted = false;
  data.onUpdate = [];
  data.hooks = createHookCollection(container);
    ...
    try {
      if (
        ...
        data.selectors.every((selector, index) =>
          isEqual(selector(context.getState(), valueOf), data.prevValues[index])
        )
      )
        return;
    } catch (e) {
  ...
  data.tryDispatchEvent = function (content, event, props) {
    if (
      content &&
      (content["$" + event] || (content.on && content.on[event]))
    ) {
      context.dispatch(content["$" + event] || content.on[event], {
        ...props,
        type: event,
        target: container,
        component
  ...
  data.forceUpdate = function () {
    ...
    const previousHookContext = hookContext;
    try {
      hookContext = data.utils;
      const content = component(data.props, data.utils);
      bind(context, container, data, content);

      if (!data.mounted) {
        data.mounted = true;
        data.tryDispatchEvent(content, "mount");
      }
      data.tryDispatchEvent(content, "update");
      const updateHandlers = data.onUpdate;
      ...
      hookContext = previousHookContext;
    }
  };
  ...
  Object.assign(data.utils, {
    useEffect(effect, deps = []) {
      return data.hooks.get(
        ...
        () => {
          let dispose;
          data.onUpdate.push(() => (dispose = effect(context)));
          return {
            dispose() {
    ...
    get(type, initial, deps = []) {
      ...
      if (!hook) {
        ...
        if (typeof hook.dispose === "function") {
          disposeCount++;
          if (!unsubscribeDestroyEvent)
            addListener(container, "destroy", handleDestroy);
        }
      }
      return hook.result();
    }
...
export function loadable(value, options = {}) {
  const props = {
    status: "loaded",
...
export function useEffect() {
  if (process.env.NODE_ENV !== "production") {
    if (!hookContext) throw new Error("No hook context found");
  }
  return hookContext.useEffect.apply(null, arguments);
}
...
export function useCallback() {
  ...
  return hookContext.useCallback.apply(null, arguments);
}
...
export function useMemo() {
  ...
  return hookContext.useMemo.apply(null, arguments);
}
...
export function useStore() {
  ...
  return hookContext.useStore.apply(null, arguments);
}
...
export function useState() {
  ...
  return hookContext.useState.apply(null, arguments);
}
...
Object.assign(hta, {
  ...
  configs(value = {}) {
    for (const prop in value) {
      switch (prop) {
        case "updateAsync":
          updateAsync = value[prop];
          break;
        case "disableLifecycle":
          disableLifecycle = value[prop];
          break;
        default:
          console.warn("Not supported config " + prop);
      }
    }
  }
});
