...
function createTemplate(strings) {
  ...
  const html = strings.reduce((prev, current, index) => {
    return `${prev} ${bindingPrefix}${index - 1}="1" ${current}`;
  });
  const slotCount = substitutions.length;
  const query = substitutions
    .map((_, index) => `[${bindingPrefix}${index}]`)
    .join(",");
  ...
  return {
    ...
    strings,
    substitutions,
    isEqual: (template) => arrayEqual(template.strings, strings),
    render(container) {
...
function init() {
  if (!document.querySelector("#hta-styles")) {
    const styleElement = document.createElement("style");
    styleElement.id = "hta-styles";
    styleElement.type = "text/css";
    const styles = `.${hideClassName} {display: none !important;}`;
